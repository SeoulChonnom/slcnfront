...
> **상태**: §1~§6은 최적화 **이전** 상태의 검증 기록이다. §7 계획은 모두 실행되었고 실측 결과는 §10에 있다.
...
§7 계획을 3단계로 구현하고 각 단계마다 실제 백엔드(`localhost:8080`)와 실제 브라우저로 검증했다.

| 커밋 | 단계 |
| --- | --- |
| `2cde7c7` | 1단계 — variant 적용 |
| `2535e4f` | 1.5단계 — 자산 캐시로 재요청 제거 |
| `811a283` | 2단계 — 뷰포트 기반 지연 로딩 |

### 10.1 전송량 (실측)

홈 최초 진입, 여행 13건 · 3024×4032 표지 기준.

| 화면 | 이전 | 1단계 후 | 3단계 후 (첫 화면) | 총 감축 |
| --- | --- | --- | --- | --- |
| 홈 데스크톱 1440 | 33,561,194 B (32.01 MiB) | 746,332 B | **443,513 B (6건)** | **-98.7%** |
| 홈 모바일 390 | 33,561,194 B (32.01 MiB) | 746,332 B | **485,298 B (7건)** | **-98.6%** |
| 여행 목록 1440 | 33,561,194 B (32.01 MiB) | 2,722,106 B | **681,664 B (4건)** | **-98.0%** |

끝까지 스크롤하면 나머지가 로드되고, 세 화면 모두 **끝내 비어 있는 이미지 자리는 0건**이다.

### 10.2 검색 재요청 (실측)

| 동작 | 이전 | 이후 |
| --- | --- | --- |
| 검색어 입력 (목록 축소) | 12건 재요청 | **0건** |
| 검색어 삭제 (목록 복원) | 12건 재요청 | **0건**, 모든 이미지가 최초 blob URL 유지 |

### 10.3 계획에서 바뀐 것

**S2-1 (`width`/`height`로 레이아웃 예약) — 실행하지 않았다.** 근거가 실측으로 부정됐다.

모든 이미지 상자가 이미 `aspect-ratio` + `object-fit: cover` 로 공간을 예약하고 `<img>` 규칙이 `width/height: 100%` 라 속성이 덮어써진다. 실제 CLS 0.144를 발생원별로 분해한 결과도 같은 결론이었다.

| 기여도 | 발생원 |
| --- | --- |
| 0.1425 | `DIV.slcn-home__secondary + NAV.slcn-home__more + FOOTER.slcn-footer` — 아카이브 목록이 렌더되며 하위 섹션을 밀어냄 |
| 0.0015 | 아카이브 행 |
| **0** | **이미지 고유 크기 누락** |

즉 이미지 치수를 넣어도 CLS는 줄지 않는다. 남은 0.14는 **목록 삽입으로 인한 레이아웃 이동**이라는 별개 문제이며 이번 범위 밖이다.

**2단계(`<img src>` 직접 사용) — 구조를 바꿔 목적만 달성했다.** §4.6에서 확인한 대로 이미지 GET은 `X-AUTH-TOKEN` 헤더가 필수라 `<img>`로 직접 인증할 수 없다. 백엔드 변경을 기다리는 대신 IntersectionObserver로 fetch 시점을 뷰포트에 연동해 지연 로딩 효과를 프론트 단독으로 얻었다.

따라서 아래는 여전히 백엔드 협조가 필요한 미해결 항목이다.

- **CORS preflight 13회** — `X-AUTH-TOKEN` 커스텀 헤더 때문이며 지연 로딩으로 건수만 줄었을 뿐 요청당 1회는 그대로다
- **Blob 메모리** — 원본/축소본을 메모리에 들고 있는 구조는 유지된다
- **브라우저 이미지 캐시(디코딩 결과) 미활용**

§7.5의 백엔드 요청 항목(대형 축소본, 구 자산 일괄 생성, `width=abc` → 400, `api_spec.json` 갱신, 쿠키 인증)은 그대로 유효하다.

### 10.4 부수적으로 고친 것

- `vite.config.ts` 의 vitest `exclude` 에 `.worktrees/**` 추가. 에이전트 워크트리를 스캔해 `pnpm test` 가 `src/` 에 도달하기 전에 실패하던 문제로, 이 변경 전에는 프로젝트 표준 검증 명령을 돌릴 수 없었다.
